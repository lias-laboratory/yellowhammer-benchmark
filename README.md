...
The prediction test dataset splits the data into two parts, with the first 19 years for estimation and the last 5 years for validation. Actually, You have access to a CSV file, [Data_Estimation_2002_2019.csv](https://forge.lias-lab.fr/datasets/yellowhammer-benchmark/Data_Estimation_2002_2019.csv.zip), containing data from the first 19 years for estimation. Validation performance is derived from the complete dataset, taking into account only the last five years.
...
Under development! You have access to some programs in [Matlab](Matlab_src/), Python, and R to facilitate the use and processing of data sets.
